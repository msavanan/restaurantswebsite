Map menuData = {
  "1": {
    "desc": "Soft, spongy idli",
    "price": 80,
    "image":
        "https://b.zmtcdn.com/data/dish_photos/e8e/efc940870ffbcbc0d73168bad92cae8e.jpg?fit=around|130:130&crop=130:130;*,*",
    "menuTitle": "Idlis [2 Pcs] + Vada"
  },
  "2": {
    "desc":
        "Buttery, tangy, super crispy, and Rava-made dosa that will you wanting more",
    "price": 60,
    "image":
        "https://b.zmtcdn.com/data/dish_photos/748/776f2830b8033a42773f3c5354165748.jpg?fit=around|130:130&crop=130:130;*,*",
    "menuTitle": "Ghee Rava Masala Dosa"
  },
  "3": {
    "desc":
        "Soft, delicious and lip-smacking baked bread prepared in typical Madurai style",
    "price": 129,
    "image":
        "https://bigoven-res.cloudinary.com/image/upload/t_recipe-256/chapati-kothu-parotta-with-leftover-chapati-recipe-1863889.jpg",
    "menuTitle": "Madurai Special Kotthu Paratha"
  },
  "4": {
    "desc":
        "Anjappar Veg Meals Enough To Feed 1 People Easily! Chettinad Flavour Anjappar Meals. The Meal Box Comes With Items:- Sweet, White Rice, Chicken Masala [2 Pcs]Sambar, Rasam, Koottu, Poriyal, Curd, Appalam, Pickle, Ghee, Dal Powder. Its Satisfying And Delicious Comes With A Banana Leaf.",
    "price": 200,
    "image":
        "https://res.cloudinary.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_208,h_208,c_fit/woozgsqbzjf7po7v2lye",
    "menuTitle": "Single Carrier Non Veg Meals"
  },
  "5": {
    "desc":
        "Medium Pepper BBQ Chicken Cheese Burst Pizza + Choco Lava Cake + Garlic Bread",
    "price": 629,
    "image":
        "https://b.zmtcdn.com/data/dish_photos/011/1dd020032f02c0e1a6043b614249c011.jpg?fit=around|130:130&crop=130:130;*,*",
    "menuTitle":
        "Ultimate cheese burst Deluxe Combo (N Veg)"
  },
  "6": {
    "desc":
        "Anjappar Mutton Curry Meals Enough To Feed 2 People Easily! Chettinad Flavour Anjappar Meals. The Meal Box Comes With Items:- Sweet, White Rice, Chettinad Mutton Masala, Sambar, Rasam, Koottu, Poriyal, Curd, Appalam, Pickle, Ghee, Dal Powder. Its Satisfying And Delicious Comes With A Banana Leaf.",
    "price": 599,
    "image":
        "https://b.zmtcdn.com/data/dish_photos/944/570d2cde651f753c1e70787fca7d7944.jpg?fit=around|130:130&crop=130:130;*,*",
    "menuTitle": "Chettinad Mutton Curry Meals"
  },
  "7": {
    "desc":
        "Shallow-Fried Anchovies [Nethili] Marinated In A Tangy, Red Pepper and Lemon Masala",
    "price": 241,
    "image":
        "https://b.zmtcdn.com/data/dish_photos/74b/0e7bbf21d81b072b40baf827e530e74b.jpg?fit=around|130:130&crop=130:130;*,*",
    "menuTitle": "Nethili Fish Fry"
  }
};
